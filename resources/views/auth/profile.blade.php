    ...
    <div class="max-w-4xl mx-auto">
        <div class="bg-white rounded-2xl shadow-lg border border-gray-200 p-8 mb-8 animate-fade-in">
            <div class="flex items-center space-x-4">
                <img src="{{ $user->imageUrl }}" alt="Profile Image" class="w-24 h-24 rounded-full object-cover border-4 border-blue-500">
                <div>
                    <h1 class="text-3xl font-semibold text-gray-800">{{ $user->name }}</h1>
                    <p class="text-xl text-gray-500">{{ $user->role }}</p>
                </div>
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
            ...
            <div class="bg-white rounded-lg shadow-lg p-6 border border-gray-200 hover:shadow-xl transition duration-300 ease-in-out">
                ...
                <p class="text-gray-600 mt-2">{{ $user->role }}</p>
            </div>
            ...
            <div class="bg-white rounded-lg shadow-lg p-6 border border-gray-200 hover:shadow-xl transition duration-300 ease-in-out md:col-span-2">
                ...
                <p class="text-gray-600 mt-2">{{ $user->doj ? $user->doj->format('M d, Y') : 'N/A' }}</p>
            </div>
        </div>

        <div class="flex justify-center md:justify-end animate-fade-in">
            <a href="{{ route('auth.profile.edit') }}" class="bg-blue-600 text-white py-2 px-6 rounded-full hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-500 transition duration-300">
